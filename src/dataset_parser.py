...
# -------------------------------------------------------
# READ FVECs (float32 vectors) ΤΟΥ ΧΡΗΣΤΟΥ
# -------------------------------------------------------
def read_fvecs(path):
...
# -------------------------------------------------------
# DATASET PARSER CLASS (needed by nlsh_search)
# -------------------------------------------------------

class DatasetParser:
    """
    Thin wrapper so nlsh_search.py can call:
        parser = DatasetParser()
        X = parser.parse(path, dtype)
    """

    def parse(self, path, dtype):
        """
        dtype: 'sift' or 'mnist'
        Uses your unified loader internally.
        """
        data, detected_type = load_dataset(path)

        # Optional check: warn if mismatch
        if dtype != detected_type and detected_type != "gt":
            print(f"[WARN] Expected dataset type '{dtype}', "
                  f"but loader detected '{detected_type}'.")
